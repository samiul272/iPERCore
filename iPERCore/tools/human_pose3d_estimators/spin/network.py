...
from iPERCore.tools.utils.geometry.rotations import rot6d_to_rotmat, rotation_matrix_to_angle_axis
...
class SPIN(nn.Module):
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3):
        ...
        # preds = torch.cat([pred_cam, pred_pose, pred_shape], dim=-1)

        pred_rotmat = rot6d_to_rotmat(pred_pose)

        pred_rotmat_hom = torch.cat(
            [pred_rotmat.view(-1, 3, 3),
             torch.tensor([0, 0, 1], dtype=torch.float32, device=device).view(1, 3, 1).expand(batch_size * 24, -1, -1)],
            dim=-1)
...
def build_spin(pretrained=False, **kwargs):
    """ Constructs an HMR model with ResNet50 backbone.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SPIN(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
