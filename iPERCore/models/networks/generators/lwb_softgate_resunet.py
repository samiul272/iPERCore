...
class SoftGateLWB(nn.Module):
    def __init__(self, in_channel, out_channel, fuse_type="add"):
        super().__init__()
    ...
    def forward(self, tsf_x, src_x, Tst, temp_x=None, Ttt=None):
        ...
        # (bs, ns, c2, h, w)
        src_warp = src_warp.view((bs, ns, -1, h, w))

        if self.fuse_type == "add":
            fused_warp = torch.sum(src_warp, dim=1)
        elif self.fuse_type == "avg":
            fused_warp = torch.mean(src_warp, dim=1)
        else:
            raise ValueError("{} must be ['add', 'mean']".format(self.fuse_type))

        # https://papers.nips.cc/paper/7329-soft-gated-warping-gan-for-pose-guided-person-image-synthesis.pdf
        gate = self.gate_conv(tsf_x)
        x = tsf_x + gate * fused_warp
...
def build_multi_stage_softgatelwb(src_num_filters, tsf_num_filters, fuse_type="add"):
    ...
    for i in range(n_down):
        block = SoftGateLWB(
            in_channel=src_num_filters[i],
            out_channel=tsf_num_filters[i],
            fuse_type=fuse_type
        )
...
def build_res_block_softgatelwb(src_res_channel, tsf_res_channel, n_res_block, fuse_type="add"):
    res_attlwbs = nn.ModuleList()
    # Down-Sampling
    for i in range(n_res_block):
        block = SoftGateLWB(
            in_channel=src_res_channel,
            out_channel=tsf_res_channel,
            fuse_type=fuse_type
        )
